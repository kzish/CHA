    ...
    public class SystemUsersController : Controller
    {
        ...
        public async Task<JsonResult> Users()
        {
            try
            {
                ...
                var users = db.AspNetUsers.Where(i => i.MCompanyIdFk == company_id).ToList();
        ...
        public async Task<JsonResult> Users([FromBody] SystemUsers user)
        {
            try
            {
                ...
                created_user.MCompanyIdFk = company_id;
        ...
        public JsonResult Users(string user_id)
        {
            try
            {
                ...
                if (user_to_delete.MCompanyIdFk == company_id)
                {
        ...
        /// <returns></returns>
        [HttpPut("Users")]
        public async Task<JsonResult> PutUsers([FromBody] SystemUsers user)
        {
            try
            {
                //
                var company_id = GetClaim("company_id");
                //
                var all_roles = db.AspNetRoles.Select(i => i.Name).ToList();
                //
                var id_user = await userManager.FindByEmailAsync(user.email);
                ...
                foreach (var role in all_roles)
                {
                    ...
                    await userManager.RemoveFromRoleAsync(id_user, role);
                }
                //all to selected roles
                await userManager.AddToRolesAsync(id_user, user.roles);
                return Json(new
                {
                    res = "ok",
                    data = "User updated"
        ...
        public async Task<JsonResult> PasswordReset(string user_id, string new_password)
        {
            try
            {
                ...
                if (company_id != user_to_reset_password.MCompanyIdFk)
                {
                    return Json(new
                    {
                        res = "err",
