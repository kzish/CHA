...
namespace admin.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        {
            var admin_user = db.AspNetUsers.Where(i=>i.Email=="admin@cha.com").FirstOrDefault();
            if (admin_user==null)
            {
                var new_admin_user = new IdentityUser();
                new_admin_user.Email = "admin@cha.com";
                new_admin_user.UserName = new_admin_user.Email;
                await userManager.CreateAsync(new_admin_user, "Password#99");//create user
                
                bool x = await roleManager.RoleExistsAsync("system_admin");
                if (!x)
                {
                    var role = new IdentityRole("system_admin");
                    await roleManager.CreateAsync(role);
                }
                await userManager.AddToRoleAsync(new_admin_user, "system_admin");//add user to admin role
            }
            return "ok";
        }
